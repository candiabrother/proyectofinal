	...
	<div class="contenido">
		<table style="margin:auto; width: 800px; border-collapse: separate; border-spacing: 10px 5px; display: inline-block; color:white;">
			<thead>
				...
				<h1 style="color: #fff; ">CARICATURAS</h1>
				<th>id</th>
				<th>nombre</th>
				...
				<th>calificacion</th>
				<th>sinopsis</th>
				<th><a href="insertarcaricatura.html"><button type="button" class="btn btn-info">Nuevo</button></a></th>
			</thead>


		<?php 
			include 'conectcrud.php';
			$sql="SELECT * FROM caricaturas";
			...
			while ($filas=mysqli_fetch_assoc($resultado)) {
				...
				echo "<td><a href='crudcaricaturas/modificar.php?id=".$filas['id']."'><button type='button' class='btn btn-success'>Modificar</button></a> </td>";
				echo "<td><a href='crudcaricaturas/eliminar.php?id=".$filas['id']."'><button type='button' class='btn btn-danger'>Eliminar</button></a></td>";
				echo "</tr>"; 
			}
		 ?>
		</table>
